function [bestGauss,bestPrData]  = mixGaussFit(data,N_GAUSS_IN,covType,nAttempts);
...
while (1)
    ...
    for (cIter = 1:N_ITER)
        fprintf('ATTEMPT %d\n',cIter);
        [gaussEst prData]  = mixGaussFitInternal(data,N_GAUSS,covType);
        if (prData==-realMax)
...
%if we have fit less Gaussians then we were asked too then add some dummy
...
function [gaussEst,prData]  = mixGaussFitInternal(data,N_GAUSS,covType);
