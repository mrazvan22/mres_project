function [parm_struct] = EBDPMCMCTestWithParamsFastFixedOrdering1K(data_controls, data_patients, parm_mcmc)
...
for roi=1:nr_events
    ...
    likelihood_events(roi, 1:nr_subj, 1) = getGaussProb(data_patients(:, roi)',thisMean_noevent(roi),sqrt(thisCov_noevent(roi)));
    likelihood_events(roi, 1:nr_subj, 2) = getGaussProb(data_patients(:, roi)',thisMean_event(roi),sqrt(thisCov_event(roi)));
end
...
for it_mcmc = 1:(nr_it_burnin + nr_it_mcmc),
    ...
    for roi=1:nr_events
        ...
        %find the current likelihood event no event
        likelihood_events_new(roi, 1:nr_subj, 1) = getGaussProb(data_patients(:, roi)',thisMean_noevent(roi),sqrt(thisCov_noevent(roi)));
        likelihood_events_new(roi, 1:nr_subj, 2) = getGaussProb(data_patients(:, roi)',thisMean_event(roi),sqrt(thisCov_event(roi)));
