function [parm_struct, diag_struct] = ...
    AtrophyModelMCMC2c(prob_pat, prob_con, span, nr_it_hillclimb, ...
...
logf_uniform_span = [-10 log(0.5)];

sig_swap = 3;
sig_f_uniform = 0.3*ones(nr_roi, 1);
...
logf_uniform_hillclimb = zeros(nr_roi, nr_hillclimb, 1);
logp_order_events_burnin = zeros(nr_it_burnin, 1);
order_events = zeros(nr_roi, nr_it_mcmc/thinning);
logf_uniform = zeros(nr_roi, nr_it_mcmc/thinning, 1);
...
for it_hillclimb = 1:nr_hillclimb,
    ...
    logf_uniform_current = (logf_uniform_span(2) - logf_uniform_span(1))*...
        rand(nr_roi, 1) + logf_uniform_span(1);
    f_local = repmat(exp(logf_uniform_current), 1, nr_pat);
    ...
    for it = 1:nr_it_hillclimb,
        ...
        for roi= 1:nr_roi,
            ...
            f_local = repmat(exp(logf_uniform_new), 1, nr_pat);            
            p_A_D = ((1-f_local).*prob_pat + f_local.*p_uniform)./...
                ((1-f_local).*prob_pat + (1-f_local).*prob_con + 2*f_local.*p_uniform);            
...
f_local = repmat(exp(logf_uniform_max), 1, nr_pat);
p_A_D = ((1-f_local).*prob_pat + f_local.*p_uniform)./...
    ((1-f_local).*prob_pat + (1-f_local).*prob_con + 2*f_local.*p_uniform);
...
f_uniform_org = zeros(nr_roi_org, size(order_events, 2));
f_uniform_org(I_nonzero, :) = exp(logf_uniform);
f_uniform_max_org = zeros(nr_roi_org, 1);
f_uniform_max_org(I_nonzero) = exp(logf_uniform_max);
