...
function [parm_struct] = EBDPMCMCTestWithParamsFastDeligate2(data_controls, data_patients, parm_mcmc)
...
for roi=1:nr_events
    ...
    init_sigma_noevent(roi)=max([sigmaMin,sigmaMax]);
    
    sigmaMin=(init_mean_event(roi)-minX)/3;
    sigmaMax=(maxX-init_mean_event(roi))/3; 
    init_sigma_event(roi)=max([sigmaMin,sigmaMax]);
    ...
    prior_thisMean_noevent(roi)=normpdf(thisMean_noevent(roi),init_mean_noevent(roi),init_sigma_noevent(roi));
    prior_thisMean_event(roi)=normpdf(thisMean_event(roi),init_mean_event(roi),init_sigma_event(roi));
