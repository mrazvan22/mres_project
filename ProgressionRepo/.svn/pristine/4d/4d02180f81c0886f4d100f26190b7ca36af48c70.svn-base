...
%     
% end
% %% calculate total data likelihood
% data=[data_patients;data_controls];
% for i=1:5
% total_like(i)=sum([gmix_struct.gmix_tot{i}.priors(1).*...
%     getGaussProb(data(:,i),gmix_struct.gmix_tot{i}.centres(1),sqrt(gmix_struct.gmix_tot{i}.covars(1)))...
%     + gmix_struct.gmix_tot{i}.priors(2).*getGaussProb(data(:,i),gmix_struct.gmix_tot{i}.centres(2),sqrt(gmix_struct.gmix_tot{i}.covars(2)))])
% end

%% calculate the data posterior
for roi=1:nr_var
    thisLike1=squeeze(likelihood(roi,:,1));
    thisLike2=squeeze(likelihood(roi,:,2));
    denom=thisLike1+thisLike2;
    posterior(roi,:,1)=thisLike1./denom;
    posterior(roi,:,2)=thisLike2./denom;
end
...
parm_struct = EBDPMCMCTestJania(likelihood, ...
    parm_mcmc);
