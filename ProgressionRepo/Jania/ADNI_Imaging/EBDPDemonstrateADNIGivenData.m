function EBDPDemonstrateADNIGivenData(data_struct,cat_nr)
%% Loading data
cnt_fig = 1;
...
nr_events = nr_roi;
I_lh = [2:2:42, 46:2:83];
I_rh = [1:2:17 21:2:47, 51:2:83];

nr_roi_lhrh = length(I_rh);
...
I_swap = [34,35,45,46,47,48,49,78,79,80,81];

avg_vol_control(I_swap,:) = -avg_vol_control(I_swap,:);
avg_vol_patient(I_swap,:) = -avg_vol_patient(I_swap,:);
...
for roi = 1:nr_roi_lhrh,
    ...
    for pat = 1:nr_pat,
        ...
        mean_local = [avg_vol_patient(I_lh(roi), pat) ...
            avg_vol_patient(I_rh(roi), pat)];
    ...
    for con = 1:nr_con,
        ...
        mean_local = [avg_vol_control(I_lh(roi), con) ...
            avg_vol_control(I_rh(roi), con)];
...
% version_mixt = 6; % Choosing the mixture of the Gaussian distributions
% opts = foptions;
% opts(14) = 1e3;
% 
...
% save(['AllRegionMoGICVNormSubdivide' num2str(cat_nr) '.mat']);



%% Visualizing results
load(['AllRegionMoGICVNormSubdivide' num2str(cat_nr) '.mat']);
