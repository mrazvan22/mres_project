...
function [parm_struct] = EBDPMCMCTestWithParams1kCorrect(data_controls, data_patients, parm_mcmc)
...
%     prior_thisMean_noevent(roi)=unifpdf(thisMean_noevent(roi),-1e10,1e10);
%     prior_thisMean_event(roi)=unifpdf(thisMean_event(roi),-1e10,1e10);
%     prior_thisCov_noevent(roi)=unifpdf(thisCov_noevent(roi),0,1e10);
%     prior_thisCov_event(roi)=unifpdf(thisCov_event(roi),0,1e10);
%     
...
%% just for testing the code do not update the parameters
...
%         
%% Option 2 : Update Samples from already accepted parameters
...
%         prior_thisMean_noevent_new(roi)=unifpdf(thisMean_noevent_new(roi),-1e10,1e10);
%         prior_thisMean_event_new(roi)=unifpdf(thisMean_event_new(roi),-1e10,1e10);
...
function [log_likelihood_tot, class_pat] = compute_loglikelihood_and_params_wrong(likelihood_events, ...
    event_order, flag_sum, prior_thisMean_noevent, ...
        prior_thisMean_event,prior_thisCov_noevent,prior_thisCov_event)
