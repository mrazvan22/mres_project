...
% variance.
%version_likelihood= 8  fits mixture of Gaussians using the gmmem package with the option of 
%thresholding the log likelihoods by setting threshold_flag=1 : similar to 6 excepts a single gaussian 
%is fitted to thecontrols and mixture fitted to patrients keeping the control component fixed                           
%version_likelihood=13  fits mixture of Gaussians similar to 8 MULTIPLE
%times from random initialization points

function [likelihood, gmix_struct] = ...
    EBDPComputeLikelihood(data_patients, data_controls, version,threshold_flag, nAttempts)
...
%         likelihood(roi,:,:)=like;
    ...
    if version==13
        ...
        [gmix_tot,prData,label]  = mixGaussFit_fixedcomp(data_tot(:, roi)',gmix_controls.mean,2,covType,nAttempts);
        ...
        if threshold_flag
            ...
            for cIndx=1:length(indx)
                thisLike=likelihood(roi,indx(cIndx),:);
                %if you are in the far left of the distributrion
                if data_patients(indx(cIndx),roi)<gmix_controls.mean
                    minIndx=find(thisLike==min(thisLike));
                    mxIndx=find(thisLike==max(thisLike));
                    thisLike(minIndx)=thisLike(mxIndx)/T;
                else
