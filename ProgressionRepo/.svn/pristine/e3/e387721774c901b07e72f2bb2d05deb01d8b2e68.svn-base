...
function [likelihood, gmix_struct] = ...
    EBDPComputeLikelihoodCorrectInit(data_patients, data_controls, version,threshold_flag, nAttempts)
...
for roi = 1:nr_roi,
    ...
    if version==8
        ...
        if gmix_tot.ncentres<2
            gmix_patients = gmm(1, 1, 'full');
            gmix_patients.centres = gmix_tot.centres(1);
            gmix_patients.covars = gmix_tot.covars(:, :, 1);
...
%          
        ...
        if threshold_flag
            ...
            for cIndx=1:length(indx)
                ...
                if data_patients(indx(cIndx),roi)<gmix_controls.centres
...
%         likelihood(roi,:,:)=like;
    ...
    if version==11
        ...
        % thoughts about better ways of setting this variable would be
        % appreciated...
        parm_mixtUnif.a = gmix_controls.centres-29*sqrt(gmix_controls.covars); 
        parm_mixtUnif.b = gmix_controls.centres;
        parm = fitFixedGaussUnif(data_tot(:, roi), parm_mixtUnif);
        parm.data = data_tot(:, roi);
        gmix_struct.gmix_roi{roi} = parm;
    ...
    if version==12
         ...
         gmix_tot=gmm(1,2,'full');
         gmix_tot=gmmem(gmix_tot,data_tot(:,roi),opts);
        if gmix_tot.ncentres<2
            gmix_tot=gmix_struct_dummy{2};
            fprintf('%s','original BIC gmix had only one component');
        end
        
        minIndx=find([gmix_tot.centres]==min(gmix_tot.centres));
        mxIndx=find([gmix_tot.centres]==max(gmix_tot.centres));
        gmix_patients = gmm(1, 1, 'full');
        gmix_patients.centres = gmix_tot.centres(minIndx);
        gmix_patients.covars = gmix_tot.covars(:, :, minIndx);
        gmix_patients.priors = 1;
        
        gmix_controls = gmm(1, 1, 'full');
        gmix_controls.centres = gmix_tot.centres(mxIndx);
        gmix_controls.covars = gmix_tot.covars(:, :, mxIndx);
        gmix_controls.priors = 1;
    ...
    if version==13
        %fit Mixture of Gaussian several times form different
        ...
        [gmix_controls,prControls]  = mixGaussFit(data_controls(:, roi)',1,covType,nAttempts);
        %initMean=[mean(data_controls(:,roi)), mean(data_patients(:,roi))];
        initMean=[0,-1];
        [gmix_tot,prData,label]  = mixGaussFit_fixedcompCorrectInit(data_tot(:, roi)',gmix_controls.mean,2,covType,nAttempts,initMean);
        
        p=[gmix_tot.prior];
        chk=find(p==0);
        
        if ~isempty(chk)
            gmix_patients.mean = gmix_controls.mean;
            gmix_patients.cov = gmix_controls.cov;
            gmix_patients.prior = 1;
            
            fprintf('%s','original BIC gmix had only one component');
        else
            gmix_patients.mean = gmix_tot(2).mean;
            gmix_patients.cov = gmix_tot(2).cov;
            gmix_patients.prior = 1;
        end
             
        likelihood(roi, :, 1) = getGaussProb(data_patients(:, roi)',gmix_controls.mean,sqrt(gmix_controls.cov));
        likelihood(roi, :, 2) = getGaussProb(data_patients(:, roi)',gmix_patients.mean,sqrt(gmix_patients.cov));
     ...
     if version==14 % same as 13 but use the netlab gmm and gmmem instead
