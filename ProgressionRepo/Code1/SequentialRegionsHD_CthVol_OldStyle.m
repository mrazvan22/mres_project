...
for flag_reg = 1:2,
    ...
    [parm_struct_lhrh{flag_reg}, diag_struct_lhrh{flag_reg}] = ...
        AtrophyModelMCMCOldStyle(p2_lhrh(:, :, flag_reg), ...
...
for roi = 1:nr_roi_lhrh,
    ...
    labels{roi} = sprintf('%02iL', roi);
    labels{roi+37} = sprintf('%02iR', roi);
...
for roi = 1:(nr_roi_lhrh),
    ...
    name_roi_lhrh{roi} = name_roi{roi}(4:end);
...
x_pos = 0;
y_pos0 = 0;
x_multiply = [0 2 3 4 5];
y_multiply = [0 2 -2 4 -4];  
dx_pos = 2;
dy_pos = 2;
figure(cnt_fig), clf, cnt_fig = cnt_fig + 1;
for roi = 1:nr_events,
