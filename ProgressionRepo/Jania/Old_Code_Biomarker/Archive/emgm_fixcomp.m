function [label, model, llh] = emgm(X, init,fix_flag)
...
R = initialization(X,init,fix_flag);
[~,label(1,:)] = max(R,[],2);
R = R(:,unique(label));
...
function R = initialization(X, init,fix_flag,n_comp)
[d,n] = size(X);
if isstruct(init)  % initialize with a model
    if fix_flag % if you need to fix a compponent
