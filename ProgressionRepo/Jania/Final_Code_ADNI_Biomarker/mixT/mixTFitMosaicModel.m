function tDist = mixTFitMosaicModel(data,N_DIST,covType,dataIDMatrix)
...
while(1)
    ...
    for(cPerson=1:N_PERSON)
        %Get the different samples of the same person in training set
        indivIndex = find(dataIDMatrix(:,cPerson));
        samples = data(:,indivIndex);
        %save the number of the samples
        N_SAMPLE=size(samples,2);
        
        for (cDist = 1:N_DIST)
            prHidden(cDist,cPerson,:) = log(tDist(cDist).prior)+ getLogTProbHidden(samples,tDist(cDist).mean,tDist(cDist).cov,tDist(cDist).dof);
        end;
...
%         prHidden = prHidden./repmat(sum(prHidden,1),N_DIST,1);
        ...
        rescaleFactor = max(prHidden(:,cPerson));
        prHidden(:,cPerson) = prHidden(:,cPerson)-rescaleFactor;
        
         %normalize to create posterior probability for each point
        prHidden(:,cPerson) = exp(prHidden(:,cPerson));
        prHidden(:,cPerson) = prHidden(:,cPerson)/sum(prHidden(:,cPerson));
