...
switch cluster_version
    case 1
        % ========= perform EM with a single biomarker only ============
        for roi=1:nr_var
            roi
            %initialize cluster centres with current csf-control and patient centres
            init_centres=[mean(data_patients(:,roi))' mean(data_controls(:,roi))'];
            this_data=data(:,roi);
            %[label, model, llh] = emgm(this_data', init_labels');
            [label, model, llh] = emgm(this_data', init_centres);
            save(['Clustering_Results/Clustering_Results_with_' name_variables{roi} '.mat'],'this_data','label','model','llh','init_centres');
        end
    case 12
        % ========= perform EM with a single biomarker only ============
        for roi=1:nr_var
            roi
            this_data=data(:,roi);
            [label, model, llh] = emgm(this_data', 2);
            save(['Clustering_Results/Clustering_Results_with_' name_variables{roi} 'Random_init.mat'],'this_data','label','model','llh');
        end
        
    case 2
        % ========= perform EM with the entire data ===================
        init_centres=[mean(data_patients)' mean(data_controls)'];
        [label, model, llh] = emgm(data', init_centres);
        save('Clustering_Results/Clustering_Results_AllData.mat');
    
    case 22
        % ========= perform EM with only selected biomarkers ===================
        I_keep=[1:3];
        this_data_patients=data_patients(:,I_keep);
        this_data_controls=data_controls(:,I_keep);
        this_data=data(:,I_keep);
        init_centres=[mean(this_data_patients)' mean(this_data_controls)'];
        [label, model, llh] = emgm(this_data', init_centres);
        save('Clustering_Results/Clustering_Results_selected_biomarkers_Data.mat');
         
    case 3
        % ============= PCA dimensionality reduced data =============
        [coefs,scores,variances,t2] = princomp(data);
        X_transformed_low_dim=scores(:,1:3);
        ...
        [label, model, llh] = emgm(X_transformed_low_dim, 2);
        save('Clustering_Results/PCA_Clustering_Results.mat');
        
    case 4 % K-means clustering
        [label,C,sumd,D] = kmeans(data,2);
        save('Clustering_Results/K-means_Clustering_Results.mat');
        %label==1 is control and label==2 patient
end
