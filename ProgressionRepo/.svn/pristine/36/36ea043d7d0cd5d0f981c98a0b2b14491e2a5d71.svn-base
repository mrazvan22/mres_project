function dataSimulationAllPermutationsDemoDeligate2
clear
%close all
...
switch sim_version
    ...
    case 2 %% case 2: Biomarkers distributions have different means but same variance
        ...
        for roi=2:nr_events
            gmix_struct_est(roi).mean_control=gmix_struct_est(1).mean_control+randn*5;
            gmix_struct_est(roi).sd_control=2.5;
            gmix_struct_est(roi).mean_patient=gmix_struct_est(1).mean_patient+randn*5;
            gmix_struct_est(roi).sd_patient=2.5;
        end
...
% Do L plots of biomarkers
figure
pCount=0;
for roi1=1:nr_events
    for roi2=roi1+1:nr_events
        pCount=pCount+1;
        subplot(2,5,pCount); plot(data_patients(:,roi1),data_patients(:,roi2),'rx');
        hold on
        plot(data_controls(:,roi1),data_controls(:,roi2),'gx');
...
set(gcf,'Color',[1 1 1]);
    ...
    version_likelihood = 8;  %version_likelihood==10 is the same as 4 but with threshold on likelihood ratio
