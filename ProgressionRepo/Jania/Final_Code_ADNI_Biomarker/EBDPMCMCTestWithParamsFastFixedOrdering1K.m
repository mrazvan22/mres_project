...
change_step_muNE=0.025;
change_step_muE=0.025;
change_step_sigNE=0.025;
change_step_sigE=0.025;
...
for it_mcmc = 1:(nr_it_burnin + nr_it_mcmc),
    ...
    for roi=1:nr_events
        ...
        %find the current likelihood event no event
        likelihood_events_new(roi, :, 1) = getGaussProb(data_patients(:, roi)',thisMean_noevent(roi),sqrt(thisCov_noevent(roi)));
        likelihood_events_new(roi, :, 2) = getGaussProb(data_patients(:, roi)',thisMean_event(roi),sqrt(thisCov_event(roi)));
