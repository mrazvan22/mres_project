...
%% combine the tau and pTau biomarker as one ans use separately
tau_controls=data_controls(:,[1,3])';
data_controls=data_controls(:,[2,4:10]);
tau_data_tot=data(:,[1,3])';
data=data(:,[2,4:10]);
name_variables={name_variables{[2,4:10]}};
name_tmp{1}='tau/pTau';
for i=1:nr_var-2
    name_tmp{i+1}=name_variables{i};
end
name_variables=name_tmp;



tau_patients=data_patients(:,[1,3])';
data_patients=data_patients(:,[2,4:10]);
...
%% calculate the joint distribution of tau and pTau
tEst_control_tau = mixTFit(tau_controls,1,1);
figure
tEst_tot_tau = mixTFitFixedComp(tau_data_tot,2,1,tEst_control_tau(1).mean,prior_control);
tEst_patient_tau=tEst_tot_tau(2);
tEst_patient_tau.prior=1;
likelihood(1, :, 1) = exp(getLogTProb(tau_patients,tEst_control_tau.mean,tEst_control_tau.cov,tEst_control_tau.dof));
likelihood(1, :, 2) = exp(getLogTProb(tau_patients,tEst_patient_tau.mean,tEst_patient_tau.cov,tEst_patient_tau.dof));
 

%% calculate likelihood for the remaining biomarkers
for roi=1:length(I_select)
    figure
    tEst_control = mixTFit(data_controls(roi,:),1,4);
    figure
    tEst_tot = mixTFitFixedComp(data(roi,:),2,4,tEst_control(1).mean,prior_control);
...
%     
    ...
    likelihood(roi+1, :, 1) = exp(getLogTProb(data_patients(roi,:),tEst_control.mean,tEst_control.cov,tEst_control.dof));
    likelihood(roi+1, :, 2) = exp(getLogTProb(data_patients(roi,:),tEst_patient.mean,tEst_patient.cov,tEst_patient.dof));
