function [parm_struct, diag_struct] = ...
    AtrophyModelMCMC2a(p_A_D, nr_it_hillclimb, ...
...
function logLik = logLikFun(X, k, theta, c, d)

[nr_events, nr_pat] = size(atrophy_patient);
nr_stage = nr_events;
likmat = zeros(nr_roi, nr_stage, nr_pat);
p_AgivenX = zeros(nr_roi, nr_pat);
p_AgivenX(X==1) = 1-c;
p_AgivenX(X==0) = d;
p_noAgivenX = zeros(nr_roi, nr_pat);
p_noAgivenX(X==1) = c;
p_noAgivenX(X==0) = 1-d;

for pat = 1:nr_pat,
    
    for stage = 1:nr_stage,
        
        for roi = 1:nr_roi,
            
            likmat = gamcdf(stage, k(roi), theta(roi))*p_AgivenX(roi, pat) + ...
                gamcdf(stage, k(roi), theta(roi))*p_noAgivenX(roi, pat);
