...
%doing log transform for ptau and tau
I_log_trans=[1,3];
data_controls(:, I_log_trans)=log(data_controls(:,I_log_trans));
data_patients(:, I_log_trans)=log(data_patients(:,I_log_trans));
...
I_select=[1,7];
log_flag=ismember(I_select,I_log_trans);
...
[likelihood, gmix_struct] = ...
    EBDPComputeLikelihoodLogTransformed(data_patients, data_controls, version_likelihood,log_flag);
