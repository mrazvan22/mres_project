...
change_step_muNE=0.0001;
change_step_muE=0.0001;
change_step_sigNE=0.0001;
...
for roi=1:nr_events
    sigma(roi)=std(data_tot(:,roi));
    if sigma(roi)<min_sigma 
        sigma(roi)=min_sigma;
    end
end
...
%% Option 2 : only samples from already accepted parameters
        ...
        thisMean_noevent_new(roi)=thisMean_noevent_current(roi)+ sigma(roi)*change_step_muNE*randn;
        thisMean_event_new(roi)=thisMean_event_current(roi)+ sigma(roi)*change_step_muE*randn;
        thisCov_noevent_new(roi)=thisCov_noevent_current(roi)+ sigma(roi)*change_step_sigNE*randn;
        thisCov_event_new(roi)=thisCov_event_current(roi)+ sigma(roi)*change_step_sigE*randn;    
