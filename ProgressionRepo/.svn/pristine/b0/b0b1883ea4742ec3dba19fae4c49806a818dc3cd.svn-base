...
nr_roi = length(I_lh);
% 
% atrophy_control_fu12.data_mean = cat(2, data_struct.data_jacc_ffd_new.data_mean(I_lh, id_group == 0, 1), ...
%     data_struct.data_jacc_ffd_new.data_mean(I_rh, id_group == 0, 1));
% I_purge = find(atrophy_control_fu12.data_mean(1, :) == 0);
% atrophy_control_fu12.data_mean(:, I_purge) = [];
% atrophy_control_fu24.data_mean = cat(2, data_struct.data_jacc_ffd_new.data_mean(I_lh, id_group == 0, 2), ...
%     data_struct.data_jacc_ffd_new.data_mean(I_rh, id_group == 0, 2));
...
flag_filt = 0;
flag_vis = 0;
[p_A_D_fu12, prob_pat_fu12, prob_con_fu12, span_fu12] = ...
    compute_p_A_D(atrophy_control_fu12, ...
    atrophy_patient_fu12, flag_mixt, flag_filt, flag_vis);
[p_A_D_fu24, prob_pat_fu24, prob_con_fu24, span_fu24] = ...
    compute_p_A_D(atrophy_control_fu24, ...
    atrophy_patient_fu24, flag_mixt, flag_filt, flag_vis);
p_A_D = cat(2, p_A_D_fu12, p_A_D_fu24);
...
%     
% end
% eval(sprintf('save dataHD_Results_NoSqueeze_021210'));

[parm_struct, diag_struct] = ...
    AtrophyModelMCMC2d(prob_pat, prob_con, span, idx_clinevent, ...
    nr_it_hillclimb, nr_it_burnin, nr_it_mcmc, thinning, nr_hillclimb, version_mcmc);
...
eval(sprintf('save dataHD_Results_Squeeze_021210'));
...
pos_roi_mean = round(mean(parm_struct.order_events, 2));
[d, order_roi_lhrh] = ...
