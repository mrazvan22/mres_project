...
for flag_reg = 1:2,
    ...
    atrophy_patient{flag_reg}.data_mean = data_struct.data_patient{flag_reg}.data_mean;
    atrophy_control{flag_reg}.data_mean = data_struct.data_control{flag_reg}.data_mean;
...
for flag_reg = 1:2,
    ...
    for pat = 1:nr_pat,
        ...
        atrophy_patient{flag_reg}.data_mean((nr_roi+1):nr_roi+data_struct.cat_patient(pat), pat) = -1;
        
    end
    atrophy_control{flag_reg}.data_mean = cat(1, atrophy_control{flag_reg}.data_mean, ...
        zeros(3, size(atrophy_control{flag_reg}.data_mean, 2)));
...
for flag_reg = 1:2,
    ...
    p1(:, :, flag_reg) = PrecedenceMatrix1(atrophy_patient{flag_reg}.data_mean');
    p2(:, :, flag_reg) = PrecedenceMatrix2(atrophy_patient{flag_reg}.data_mean', ...
...
for flag_reg = 1:2,
    ...
    for hem = 1:2,
        ...
        [parm_struct_hem{hem}{flag_reg}, ...
            ...
            AtrophyModelMCMCOldStyle(p2_hem(:, :, flag_reg, hem), ...
            nr_it_hillclimb, nr_hillclimb, nr_it_burnin, nr_it_mcmc, thinning);

    end
    [parm_struct{flag_reg}, diag_struct{flag_reg}] = ...
        AtrophyModelMCMCOldStyle(p2(:, :, flag_reg), ...
        nr_it_hillclimb, nr_hillclimb, nr_it_burnin, nr_it_mcmc, thinning);
...
eval(sprintf('save dataADResultsoldStyle%d', flag_mixt));
...
for flag_reg = 1:2,
    ...
    mean_order(:, flag_reg) = ...
        mean(order_events(:, :, flag_reg), 2);
    std_order(:, flag_reg) = ...
        std(order_events(:, :, flag_reg), [], 2);
...
for flag_reg = 1:2,
    ...
    mean_order(:, flag_reg) = ...
        mean(order_events_lhrh(:, :, flag_reg), 2);
    std_order(:, flag_reg) = ...
        std(order_events_lhrh(:, :, flag_reg), [], 2);
...
position_roi_mean = mean(order_events(:, :, flag_reg), 2);
position_roi_std = std(order_events(:, :, flag_reg), [], 2);
