...
function [parm_struct] = EBDPMCMCTestWithParams(data_controls, data_patients, parm_mcmc)
...
% thisMean_noevent=zeros(1,nr_events);
% thisMean_event=zeros(1,nr_events);
% thisCov_noevent=zeros(1,nr_events);
% thisCov_event=zeros(1,nr_events);
% 
% init_mean_noevent=zeros(1,nr_events);
% init_mean_event=zeros(1,nr_events);
% init_cov_noevent=zeros(1,nr_events);
% init_cov_event=zeros(1,nr_events);
% 
% prior_sigma_noevent=zeros(1,nr_events);
% prior_sigma_event=zeros(1,nr_events);
% 
% prior_thisMean_noevent=zeros(1,nr_events);
% prior_thisMean_event=zeros(1,nr_events);
% prior_thisCov_noevent=zeros(1,nr_events);
% prior_thisCov_event=zeros(1,nr_events);
% 
% 
% thisMean_noevent_current=zeros(1,nr_events);
% thisMean_event_current=zeros(1,nr_events);
% thisCov_noevent_current=zeros(1,nr_events);
% thisCov_event_current=zeros(1,nr_events);
% 
% 
% thisMean_noevent_new=zeros(1,nr_events);
% thisMean_event_new=zeros(1,nr_events);
% thisCov_noevent_new=zeros(1,nr_events);
% thisCov_event_new=zeros(1,nr_events);
% 
% prior_thisMean_noevent_new=zeros(1,nr_events);
% prior_thisMean_event_new=zeros(1,nr_events);
% prior_thisCov_noevent_new=zeros(1,nr_events);
% prior_thisCov_event_new=zeros(1,nr_events);
...
for it_gradient_ascent = 1:nr_gradient_ascent,
    ...
    log_likelihood_current = compute_loglikelihood(likelihood_events_est, ...
        event_order_current, flag_sum);
    for it = 1:nr_it_gradient_ascent,
        ...
        log_likelihood_new = compute_loglikelihood(likelihood_events_est, ...
...
for roi=1:nr_events
%     thisMean_noevent(roi)=gmix_struct_est.gmix_controls{roi}.centres;
%     thisMean_event(roi)=gmix_struct_est.gmix_patients{roi}.centres;
%     thisCov_noevent(roi)=gmix_struct_est.gmix_controls{roi}.covars;
%     thisCov_event(roi)=gmix_struct_est.gmix_patients{roi}.covars;
...
%     % Set the prior over variance to be a unifrom prior
    ...
    %% ============= Option 3 ======================
    % Set all priors to be a unifrom prior
%     prior_thisMean_noevent(roi)=unifpdf(thisCov_noevent(roi),0,1e10);
%     prior_thisMean_event(roi)=unifpdf(thisCov_noevent(roi),0,1e10);
%     prior_thisCov_noevent(roi)=unifpdf(thisCov_noevent(roi),0,1e10);
%     prior_thisCov_event(roi)=unifpdf(thisCov_event(roi),0,1e10);
%     
...
% option 1 compute the likelihood starting with k=0 i.e. no event has ahapped
...
% %% option 2 compute likelihood as the standard EBDP model with k=1 start
% log_likelihood_current=compute_loglikelihood_and_params(likelihood_events, ...
%     event_order_current, flag_sum, prior_thisMean_noevent, ...
%         prior_thisMean_event,prior_thisCov_noevent,prior_thisCov_event);
% 
% 
...
change_step_muNE=0.2;
change_step_muE=0.2;
change_step_sigNE=0.2;
change_step_sigE=0.2;
...
%         thisMean_noevent_new(roi)=thisMean_noevent_current(roi);
%         thisMean_event_new(roi)=thisMean_event_current(roi);
%         thisCov_noevent_new(roi)=thisCov_noevent_current(roi);
%         thisCov_event_new(roi)=thisCov_event_current(roi);    
%         
%         prior_thisMean_noevent_new(roi)=prior_thisMean_noevent(roi);
%         prior_thisMean_event_new(roi)=prior_thisMean_event(roi);
%         prior_thisCov_noevent_new(roi)=prior_thisCov_noevent(roi);
%         prior_thisCov_event_new(roi)=prior_thisCov_event(roi);
%         
        ...
        %find the current likelihood event no event
        likelihood_events_new(roi, 1:nr_subj, 1) = getGaussProb(data_tot(:, roi)',thisMean_noevent_new(roi),sqrt(thisCov_noevent_new(roi)));
        likelihood_events_new(roi, 1:nr_subj, 2) = getGaussProb(data_tot(:, roi)',thisMean_event_new(roi),sqrt(thisCov_event_new(roi)));
    ...
    %% option 1 compute likelihood starting with k=0
...
%% option 2 compute likelihood as standard EBDP model with k=1 start

%     log_likelihood_new=compute_loglikelihood_and_params(likelihood_events_new, ...
%     event_order_new, flag_sum, prior_thisMean_noevent_new, ...
%         prior_thisMean_event_new,prior_thisCov_noevent_new,prior_thisCov_event_new);
%     
%     
