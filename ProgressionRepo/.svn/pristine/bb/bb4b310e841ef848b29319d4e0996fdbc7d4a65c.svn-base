...
sim_version=1;
mixT_flag=0;     % if set to 1 it will set a minimum threshold for the log likelihood ratios
noise_flag=0;    % if not 0 it will add random noise to the data in generation
                 % noise_flag = 1: Add n% of the data patients to the data controls
                 % noise_flag = 2: Add random uniform noise to the patients
nois_pct=[0:0.05:0.95];  %set the percentage of the noise
rand('seed',1);
randn('seed',1);
flag_visualize=1;
...
% option 2 : randomely choose the patinet stage
...
for cItr=1:itr
    ...
    % I also find the analytical solution to estimate the ordering without
    % using the MCMC st ep. NOTE: est_order_gauss: represents the ordering
    % of events i.e. est_order_gauss= [1 2 3] this means the ordering is
    % Event_1, Event_2 and Event _3 have occured in this order.
    est_order_gauss=findNEventOrdering(likelihood);
    
    % for comparison I also give the option of estimating the ordering
    % using a Mixture of T distributions to estimate the likelihoods.
    if mixT_flag
        min_prior_flag=1;
        [likelihood_mixT,gmix_struct_mixT_est]=computeMixTLikelihood(data_controls',data_patients',threshold_flag,min_prior_flag);
        est_order_mixT=findNEventOrdering(likelihood_mixT);
        
    end
    
    %     %% save the data and the results
    %     data_controls_allItr(cItr,:,:)=data_controls;
    %     data_patients_allItr(cItr,:,:)=data_patients;
    %      est_order_gauss_allItr(cItr,:)=est_order_gauss(1,:);
    %    est_order_mixT_allItr(cItr,:)=est_order_mixT(1,:);
    %
end % end for itr

if flag_visualize
    % show the estiated means and standard deviations
%     for i=1:nr_events
%         mc(i)=gmix_struct_est.gmix_controls{i}.centres;
%         sdc(i)=sqrt(gmix_struct_est.gmix_controls{i}.covars);
%         fc(i)=gmix_struct_est.gmix_tot{i}.priors(1);
%         mp(i)=gmix_struct_est.gmix_patients{i}.centres;
%         sdp(i)=sqrt(gmix_struct_est.gmix_patients{i}.covars);
%         p=gmix_struct_est.gmix_tot{i}.priors;
%         if length(p)>1
%             fp(i)=gmix_struct_est.gmix_tot{i}.priors(2);
%         else
...
%     end
    ...
    % show the estiated means and standard deviations
    for i=1:nr_events
        mc(i)=gmix_struct_est.gmix_controls{i}.mean;
        sdc(i)=sqrt(gmix_struct_est.gmix_controls{i}.cov);
        mp(i)=gmix_struct_est.gmix_patients{i}.mean;
        sdp(i)=sqrt(gmix_struct_est.gmix_patients{i}.cov);
        p=[gmix_struct_est.gmix_tot{i}.prior];
        if length(p)>1
            fc(i)=p(1);
            fp(i)=p(2);
        else
    ...
    % for i=1:itr
    % diff_gauss(i)=sum(abs(true_order-est_order_gauss_allItr(i,:)));
    % diff_mixT(i)=sum(abs(true_order-est_order_mixT_allItr(i,:)));
    % end
    % pct_correct_gauss=length(find(diff_gauss==0))/itr*100
    % pct_correct_mixT=length(find(diff_mixT==0))/itr*100
    %
    ...
    for i_var = 1:nr_events,
        ...
        figure;
%         p_controls = gmmprob(gmix_struct_est.gmix_controls{i_var}, data_controls(:, i_var));
%         p_patients = gmmprob(gmix_struct_est.gmix_patients{i_var}, data_patients(:, i_var));
        p_controls = mixGaussLogProb(data_controls(:, i_var)',gmix_struct_est.gmix_controls{i_var})
        p_patients = mixGaussLogProb(data_patients(:, i_var)',gmix_struct_est.gmix_patients{i_var})
    ...
    for roi1=1:nr_events
        for roi2=roi1+1:nr_events
            pCount=pCount+1;
            h1=subplot(2,5,pCount);plot(data_patients(:,roi1),data_patients(:,roi2),'rx');
