function tDist = mixTFitMosaicModelFast(data,N_DIST,covType,dataIDMatrix)
...
%=====================NOTICE============================
% I get very large det cov i.e infinity i.e prod(covar)=inf!! why?? for now
% i replaced it with realmax
%=============================================================
...
prHidden = zeros(N_DIST,N_PERSON);
for (cDist = 1:N_DIST)
    c=find(clusters==cDist);
    [x y]=find(dataIDMatrix(c,:));
    prHidden(cDist,y)=1;
...
while(1)
    ...
    %============================= M-STEP ==================================
    %=======================================================================
    %replicate the posterior probabilities for each data point instead of
    %each individual
    extPrHidden = prHidden*dataIDMatrix';
    ...
    for (cDist = 1:N_DIST)
         tDist(cDist).mean = sum(repmat(expectedU(cDist,:).*extPrHidden(cDist,:),N_DIM,1).*data,2);
        tDist(cDist).mean = tDist(cDist).mean/sum(expectedU(cDist,:).*extPrHidden(cDist,:),2);
    ...
    if (covType==0|covType==2|covType==4)%    - if individual
        for (cDist = 1:N_DIST)
            ...
            elseif(covType==2) % diagonal covariance
                extPrHidden(extPrHidden==0)=realmin;
                %tDist(cDist).cov = sum((repmat(extPrHidden(cDist,:).*expectedU(cDist,:),N_DIM,1).*xMinusMean).*xMinusMean,2);
                cc=sum((repmat(extPrHidden(cDist,:).*expectedU(cDist,:),N_DIM,1).*xMinusMean).*xMinusMean,2);
                cc(cc==0)=realmin;
                tDist(cDist).cov=cc;
            ...
            tDist(cDist).cov = tDist(cDist).cov/sum(extPrHidden(cDist,:));
        end;
        ...
        for (cDist = 1:N_DIST)
            ...
            if (covType==1) %full covariance
                for(cData = 1:N_DATA)
                    tDist(1).cov = tDist(1).cov+exPrHidden(cDist,cData)*expectedU(cDist,cData)*xMinusMean(:,cData)*xMinusMean(:,cData)';
                end;
...
%                tDist(1).cov = tDist(1).cov+sum((repmat(extPrHidden(cDist,:).*expectedU(cDist,:),N_DIM,1).*xMinusMean).*xMinusMean,2);
                ...
                tDist(1).cov = tDist(1).cov+sum(sum((repmat(extPrHidden(cDist,:).*expectedU(cDist,:),N_DIM,1).*xMinusMean).*xMinusMean,2))/N_DIM;
            end;
        end;
        %normalize
    ...
    for (cDist = 1:N_DIST)
        estNu= fminbnd(@(x) optCrit(x,expectedLogU(cDist,:),expectedU(cDist,:),extPrHidden(cDist,:)),-1,5);
    ...
    %collect samples of each individual
       
        for (cDist = 1:N_DIST)
            prHidden(cDist,:) = log(tDist(cDist).prior)+ getLogTProbSamples(data,tDist(cDist).mean,tDist(cDist).cov,tDist(cDist).dof,sampleSizes,N_PERSON);
        end;
...
function rt= getLogTProbSamples(data,meanVal,covar,dof,sampleSizes,N_PERSON)
...
for(cPerson=1:N_PERSON)
    sampleLogProb(cPerson)=sum(logProb(cData:cData+sampleSizes(cPerson)-1));
...
function r=getLogTProbHidden(data,mean,covar,dof)

 [N_DIM N_DATA] = size(data);
 xMinusMean =data-repmat(mean,1,N_DATA);
 covarSq=repmat(covar,1,N_DATA);

nDim = size(data,1);

if (size(covar,2)==1)%if cov is diagonal
    logProb = -0.5*nDim*log(2*pi)-0.5*sum(covarSq)-0.5*sum(((xMinusMean).^2)./covarSq,1);
else %if it is general (slower)
    logProb= -0.5*nDim*log(2*pi)-0.5*log(det(covarSq))-0.5*(xMinusMean)'*inv(covarSq)*(xMinusMean);
end;
    

%logPrSamples=sum(prSamples);

prSamples=sum(logProb);
r=prSamples;
%=======================================================================
...
function r=optCrit(logNu,expectedLogU, expectedU,extPrHidden)
...
extPrHidden(extPrHidden==0)=realmin;
extPrHidden = length(extPrHidden)*extPrHidden/sum(extPrHidden);

totalWeight = sum(extPrHidden);

expectedLogU = (expectedLogU.*extPrHidden);
expectedU = (expectedU.*extPrHidden);
