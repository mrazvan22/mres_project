...
function [likelihood, gmix_struct] = ...
    EBDPComputeLikelihoodLogTransformed(data_patients, data_controls, version,log_flag)
...
for roi = 1:nr_roi,
    ...
    if version==8
        ...
        gmix_patients.priors = 1;
         ...
         if log_flag(roi)
             gmix_controls.centres=exp(gmix_controls.centres);
             gmix_controls.covars=100*(exp(gmix_controls.covars)-1);
             gmix_patients.centres=exp(gmix_patients.centres);
             gmix_patients.covars=100*(exp(gmix_patients.covars)-1);
             data_patients(:, roi)=exp(data_patients(:, roi));
         end
        ...
        %distribution with a fixed likelihood for all data
         patient_sample=gmix_patients.centres+2*sqrt(gmix_patients.covars);
         min_acc_like_patient=gmmprob(gmix_patients,patient_sample);
       
        
         
         like2=likelihood(roi, :, 2);
         %like2(like2<min_acc_like_patient)=min_acc_like_patient;
         %likelihood(roi, :, 2)=like2;
         like2(like2<1e-3)=1e-3;
         likelihood(roi, :, 2)=like2;
