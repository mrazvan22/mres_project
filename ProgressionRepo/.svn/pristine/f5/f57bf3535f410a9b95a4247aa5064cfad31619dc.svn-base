function dataSimulationDemoDeligate2
clear
%close all
...
nr_controls=46;     % select the nuber of controls
sim_version=1;
...
switch sim_version
    ...
    case 1 %% case 1: Simple case where control and patient distributions are the same for all biomarkers
        ...
        for roi=1:nr_events
            ...
            gmix_struct(roi).mean_patient=-0.5;
...
%                     noise_indx=perm(1:round(nois_pct(cItr)*nr_controls));
    ...
    version_likelihood = 8 ;  %version_likelihood==13 is the same as 8 but with a different GMM package
    [likelihood, gmix_struct_est] = ...
        EBDPComputeLikelihood(data_patients, data_controls, version_likelihood,threshold_flag);
