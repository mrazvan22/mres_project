...
% for roi=1:10
%     roi
%     [label, model] = EMMixT(data(roi,:), 2,covType);
%     m=[model.mean];
%     control_label=find(m==max(m));
%     patient_label=find(m==min(m));
%     patient_indx=find(label==patient_label);
%     control_indx=find(label==control_label);
%     data_controls_est=data(roi,control_indx);
%     data_patients_est=data(roi,patient_indx);
...
%     plot(x_p, hist_p, 'r');
%     
%     [hist_c, x_c] = ksdensity(data_controls_est);
%     [hist_p, x_p] = ksdensity(data_patients_est);
%     subplot(122), hold on
%     plot(x_c, hist_c, 'g.');
%     plot(x_p, hist_p, 'r.');
% end
