function [bestGauss,bestPrData,label]  = mixGaussFit_fixedcompCorrectInit(data,fix_mean,N_GAUSS_IN,covType,nAttempts,initMean);
...
while (1)
    ...
    for (cIter = 1:N_ITER)
        fprintf('ATTEMPT %d\n',cIter);
        [gaussEst allPrData(cIter),label]  = mixGaussFitInternal(data,fix_mean,N_GAUSS,covType,initMean);
...
function [gaussEst,prData,label]  = mixGaussFitInternal(data,fix_mean,N_GAUSS,covType,initMean);
