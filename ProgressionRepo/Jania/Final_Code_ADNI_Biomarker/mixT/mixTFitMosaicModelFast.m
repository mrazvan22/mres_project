...
while(1)
    ...
    %collect samples of each individual
        ...
        for (cDist = 1:N_DIST)
            prHidden(cDist,:) = log(tDist(cDist).prior)+ getLogTProbSamples(data,tDist(cDist).mean,tDist(cDist).cov,tDist(cDist).dof,N_PERSON);
        end;
...
function sampleLogProb= getLogTProbSamples(data,meanVal,covar,dof,N_PERSON)

 [N_DIM N_DATA] = size(data);
 xMinusMean =data-repmat(meanVal,1,N_DATA);
 N_SAMPLE=N_DATA/N_PERSON;
 ...
 logProb = logProb-(dof/2+N_DIM/2)*log(1+deltaVal/dof);
  ...
  count=0;
%sum over samples
for(cData=1:N_SAMPLE:N_DATA)
    if(N_SAMPLE==2)
        count=count+1;
        sampleLogProb(count)=sum(logProb(cData:cData+1));
    else
        sampleLogProb=0;
    end
end
