...
function [parm_struct] = EBDPMCMCTestWithParams1k(data_controls, data_patients, parm_mcmc,version_likelihood,threshold_flag,nAttempts)
...
allMeanENew=zeros(nr_it_mcmc+nr_it_burnin,nr_events);
allMeanNENew=zeros(nr_it_mcmc+nr_it_burnin,nr_events);
allCovENew=zeros(nr_it_mcmc+nr_it_burnin,nr_events);
allCovNENew=zeros(nr_it_mcmc+nr_it_burnin,nr_events);
...
for roi=1:nr_events
    ...
    %% set the parameters of the priors over Mu and Sigma
...
for roi=1:nr_events
    ...
    if threshold_flag
        T=[3.2];
        ratio=abs(log(likelihood_events(roi, :, 1))-log(likelihood_events(roi, :, 2)));
        indx=find(ratio>log(T));
        
        for cIndx=1:length(indx)
            thisLike=likelihood_events(roi,indx(cIndx),:);
            %if you are in the far left of the distributrion
            if data_patients(indx(cIndx),roi)<thisMean_noevent(roi)
            ...
            likelihood_events(roi,indx(cIndx),:)=thisLike;
        end
    end
...
change_step_muNE=0.005;
change_step_muE=0.005;
change_step_sigNE=0.005;
change_step_sigE=0.005;

min_sigma=0.09;

% calculate a proposal distribution in this case based on the variance of the total data
for roi=1:nr_events
    sigma_event(roi)=std(data_patients(:,roi));
    if sigma_event(roi)<min_sigma
        sigma_event(roi)=min_sigma;
    end
    sigma_noevent(roi)=std(data_controls(:,roi));
...
for it_mcmc = 1:(nr_it_burnin + nr_it_mcmc),
    ...
    for roi=1:nr_events
        ...
        %*** NOTE *** because some of the biomarkers have very small
        %variance the sampling step results in a negative value.  hence I
        %use the absolute value of the sample
        ...
        thisCov_noevent_new(roi)=abs(thisCov_noevent_current(roi)+ sigma_noevent(roi)*change_step_sigNE*randn);
        thisCov_event_new(roi)=abs(thisCov_event_current(roi)+ sigma_event(roi)*change_step_sigE*randn);
        ...
        %find the current likelihood event no event
        likelihood_events_new(roi, :, 1) = getGaussProb(data_patients(:, roi)',thisMean_noevent_new(roi),sqrt(thisCov_noevent_new(roi)));
        likelihood_events_new(roi, :, 2) = getGaussProb(data_patients(:, roi)',thisMean_event_new(roi),sqrt(thisCov_event_new(roi)));
        
        
        
        if threshold_flag
            ratio=abs(log(likelihood_events_new(roi, :, 1))-log(likelihood_events_new(roi, :, 2)));
            indx=find(ratio>log(T));
            
            for cIndx=1:length(indx)
                thisLike=likelihood_events_new(roi,indx(cIndx),:);
                %if you are in the far left of the distributrion
                if data_patients(indx(cIndx),roi)<thisMean_noevent_new(roi)
                    minIndx=find(thisLike==min(thisLike));
                    mxIndx=find(thisLike==max(thisLike));
                    thisLike(minIndx)=thisLike(mxIndx)/T;
                else
                ...
                likelihood_events_new(roi,indx(cIndx),:)=thisLike;
            end
        end
    ...
    %mu and sigma: compute likelihood as standard EBDP model with k=1 start
    ...
    allMeanENew(it_mcmc,:)=thisMean_event_new;
    allMeanNENew(it_mcmc,:)=thisMean_noevent_new;
    allCovENew(it_mcmc,:)=thisCov_event_new;
    allCovNENew(it_mcmc,:)=thisCov_noevent_new;
...
parm_struct.acceptance_rate=accept_count/nr_it_mcmc
...
likelihood_nonclinevents(likelihood_nonclinevents == 0) = ...
    min(likelihood_nonclinevents(likelihood_nonclinevents ~= 0));
log_likelihood_nonclinevents = log(likelihood_nonclinevents);
prior_thisMean_event(prior_thisMean_event==0)=realmin;
prior_thisMean_noevent(prior_thisMean_noevent==0)=realmin;
prior_thisCov_noevent(prior_thisCov_noevent==0)=realmin;
prior_thisCov_event(prior_thisCov_event==0)=realmin;
